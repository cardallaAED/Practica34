...
```{r}
pew_religion.t<-pew_religion%>%gather(key = "income", value = "freq",-religion)
...
head(raw[, 1:12])
billboard.t<-raw%>%gather(key = "week",value = "rank",starts_with("wk"))
...
library(tidyr)
library(dplyr)
billboar.t.pl<-raw%>%pivot_longer(names_to ="week",values_to = "rank",starts_with("wk"),names_prefix ="wk", values_drop_na = TRUE )%>%
              mutate(week=as.numeric(week),date=as.Date(date.entered)+(week - 1) * 7)%>%arrange(rank)
billboar.t.pl
```
...
```{r}
raw%>%gather(key="sexo_edad",value = "casos",starts_with(c("m","f")) )%>%
...
```{r}
tiempo.t<-raw%>%pivot_longer(names_to = "dia", values_to = "valor",starts_with("d") ,names_prefix = "d" )%>%
  mutate(dia=as.numeric(dia),fecha=as.Date(paste(year,month,dia,sep='-')))%>%drop_na(valor)%>%
  pivot_wider(names_from = element,values_from = valor)%>%select(-year,-month,-dia)
```
# DPLYR 
...
planes.f<-planes%>%filter(!str_detect(str_to_lower(manufacturer),'.*douglas.*'))%>%
  filter(between(seats,90,200))%>%
  mutate(manufacture=if_else(str_detect(str_to_lower(manufacturer),'.*airbus.*'),"AIRBUS COMP",manufacturer))
```
...
```{r }
flights.f<-flights %>%inner_join(planes%>%select(tailnum), by="tailnum")
...
```{r }
flights_e<-flights%>%select(-time_hour)%>%
                    select(!ends_with("time"))
                
planes_e<-planes%>%select(-speed,-type)                 
...
```{r }
flights_date<-flights%>%mutate(FullDate = as.character(paste(day,month,year,sep = "-")))%>%
...
Distances_plane<- flights%>% group_by(tailnum)%>%
                    summarise(Num_fligths=n(),#funciones de agregacion
...
billboard_tidy_indice<-billboar.t.pl%>%distinct()%>%mutate(id=row_number())%>%select(id, everything())
canciones<- billboard_tidy_indice%>%select(id,artist,track,time)
ranking<-billboard_tidy_indice%>%select(id,rank,week)

billboard_tidy_indice<-billboar.t.pl%>%unique()%>%mutate(id=row_number())%>%select(id, everything())
canciones<- billboard_tidy_indice%>%select(id,artist,track,time)
ranking<-billboard_tidy_indice%>%select(id,rank,week)
```
...
conversion<-table5%>%mutate(year=paste0(century,year))%>%
  select(-century)%>%
...
```{r }
Distance_Manufacturer_model<-inner_join(Distances_plane,planes,by="tailnum")%>%
...
```{r }
retrasos<- inner_join(flights,planes%>%select(tailnum,manufacturer),by="tailnum")%>%group_by(manufacturer)%>%summarise(retrasos_llegada=mean(arr_delay,na.rm=TRUE),
                                                                                                                       retrasos_salidas=mean(dep_delay,na.rm=TRUE))%>%arrange(desc(retrasos_salidas))
#¿Cuáles son los dos fabricantes de aviones que más se retrasan en las salidas?
#
tail(retrasos,2)
#¿Y en las llegadas?
tail(retrasos%>%arrange(desc(retrasos_llegada)),2)
```
